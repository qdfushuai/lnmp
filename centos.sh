...
clear
echo "========================================================================="
echo "LTMP V1.0 for CentOS/RadHat Linux"
echo "###########################################################"
echo "## SYSTEM REQUIRED:  Linux                               ##" 
echo "## Professional: Ebandao revise                          ##"
echo "## website: http://www.ebandao.cn    /                   ##"
echo "###########################################################"
echo "========================================================================="
cur_dir=$(pwd)
...
echo "==========================="
	...
	isinstallmysql55="n"
	echo "Install MySQL 5.5.33,Please input y"
	echo "Install MySQL 5.1.70,Please input n or press Enter"
	read -p "(Please input y or n):" isinstallmysql55

	case "$isinstallmysql55" in
	y|Y|Yes|YES|yes|yES|yEs|YeS|yeS)
	echo "You will install MySQL 5.5.33"
	isinstallmysql55="y"
	;;
	n|N|No|NO|no|nO)
	echo "You will install MySQL 5.1.70"
	isinstallmysql55="n"
	;;
	*)
	echo "INPUT error,You will install MySQL 5.1.70"
	isinstallmysql55="n"
	esac
...
function InitInstall()
{
	...
	cp /etc/yum.conf /etc/yum.conf.ltmp
...
if [ "$isinstallphp53" = "n" ]; then
	if [ -s php-5.2.17.tar.gz ]; then
	  ...
	  wget -c http://www.php.net/distributions/php-5.2.17.tar.gz
	fi
	...
	if [ -s php-5.3.27.tar.gz ]; then
	  ...
	  wget -c http://www.php.net/distributions/php-5.3.27.tar.gz
	fi
fi
...
if [ -s Tengine-1.5.0.tar.gz ]; then
  echo "Tengine-1.5.0.tar.gz [found]"
  else
  echo "Error: Tengine-1.5.0.tar.gz not found!!!download now......"
  wget -c http://tengine.taobao.org/download/tengine-1.5.0.tar.gz
fi

if [ "$isinstallmysql55" = "n" ]; then
	if [ -s mysql-5.1.70.tar.gz ]; then
	  echo "mysql-5.1.70.tar.gz [found]"
	  else
	  echo "Error: mysql-5.1.70.tar.gz not found!!!download now......"
	  wget -c http://mirrors.sohu.com/mysql/MySQL-5.1/mysql-5.1.70.tar.gz
	fi
else
	if [ -s mysql-5.5.33.tar.gz ]; then
	  echo "mysql-5.5.33.tar.gz [found]"
	  else
	  echo "Error: mysql-5.5.33.tar.gz not found!!!download now......"
	  wget -c http://mirrors.sohu.com/mysql/MySQL-5.5/mysql-5.5.33.tar.gz
	fi
fi
...
if [ -s p.tar.gz ]; then
  ...
  wget -c http://ip.ebandao.cn/p.tar.gz
fi
...
if [ -s autoconf-2.13.tar.gz ]; then
  ...
  wget -c http://mirrors.kernel.org/gnu/autoconf/autoconf-2.13.tar.gz
fi
...
if [ `getconf WORD_BIT` = '32' ] && [ `getconf LONG_BIT` = '64' ] ; then
	wget -c http://download.savannah.gnu.org/releases/libunwind/libunwind-1.1.tar.gz
        ...
        cd libunwind-1.1
        ./configure   --enable-shared --enable-frame-pointer
        make && make install
...
{
echo "============================Install MySQL 5.1.70=================================="
cd $cur_dir
rm -f /etc/my.cnf
tar zxvf mysql-5.1.70.tar.gz
cd mysql-5.1.70/
if [ $installinnodb = "y" ]; then
./configure --prefix=/usr/local/mysql --with-extra-charsets=complex --enable-thread-safe-client --enable-assembler --with-mysqld-ldflags=-all-static --with-charset=utf8 --enable-thread-safe-client --with-big-tables --with-readline --with-ssl --with-embedded-server --enable-local-infile --with-mysqld-ldflags=-ltcmalloc --with-plugins=innobase
else
./configure --prefix=/usr/local/mysql --with-extra-charsets=complex --enable-thread-safe-client --enable-assembler --with-mysqld-ldflags=-all-static --with-charset=utf8 --enable-thread-safe-client --with-big-tables --with-readline --with-ssl --with-embedded-server --enable-local-infile --with-mysqld-ldflags=-ltcmalloc 
fi
make && make install
cd ../
...
rm -f /usr/local/mysql/bin/mysqld_safe
cp /root/ltmp1.0/conf/mysqld5.1_safe /usr/local/mysql/bin/mysqld_safe
chown -R mysql:mysql /usr/local/mysql

/etc/init.d/mysql restart
/etc/init.d/mysql stop
echo "============================MySQL 5.1.70 install completed========================="
}

function InstallMySQL55()
{
echo "============================Install MySQL 5.5.33=================================="
cd $cur_dir
...
rm -f /usr/local/mysql/bin/mysqld_safe
cp /root/ltmp1.0/conf/mysqld5.5_safe /usr/local/mysql/bin/mysqld_safe
chown -R mysql:mysql /usr/local/mysql

/etc/init.d/mysql restart
/etc/init.d/mysql stop
echo "============================MySQL 5.5.33 install completed========================="
}
...
cd php-5.2.17/
wget -c http://ip.ebandao.cn/php-5.2.17-max-input-vars.patch
...
wget -c http://ip.ebandao.cn/init.d.php-fpm5.2
cp init.d.php-fpm5.2 /etc/init.d/php-fpm
chmod +x /etc/init.d/php-fpm
cp $cur_dir/ltmp /root/ltmp
chmod +x /root/ltmp
...
cp $cur_dir/php-5.3.27/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm
chmod +x /etc/init.d/php-fpm

cp $cur_dir/ltmp /root/ltmp
chmod +x /root/ltmp
sed -i 's:/usr/local/php/logs:/usr/local/php/var/run:g' /root/ltmp
echo "============================PHP 5.3.27 install completed======================"
}
...
cd tengine-1.5.0/
./configure --user=www --group=www --prefix=/usr/local/nginx --with-http_stub_status_module --with-http_ssl_module --with-http_gzip_static_module --with-ipv6 --with-http_concat_module --with-google_perftools_module --with-http_sysguard_module
make && make install
cd ../
...
sed -i 's/EBANDAOCN/EBANDAO.cn'$RANDOM'ebandao.cn/g' /home/wwwroot/default/phpmyadmin/config.inc.php
...
{
echo "============================add Tengine and php-fpm on startup============================"
echo "Download new Tengine init.d file......"
wget -c http://ip.ebandao.cn/init.d.nginx
...
echo "===========================add Tengine and php-fpm on startup completed===================="
echo "Starting LTMP..."
/etc/init.d/mysql start
...
if [ -s /usr/local/nginx ] && [ -s /usr/local/nginx/sbin/nginx ]; then
  ...
  echo "Error: /usr/local/nginx not found!!!Tengine install failed."
fi
...
echo "Install LTMP 1.0 completed! enjoy it."
echo "========================================================================="
echo "LTMP V1.0 for CentOS/RadHat Linux Written by Ebandao.cn"
echo "###########################################################"
echo "## SYSTEM REQUIRED:  Linux                               ##" 
echo "## AUTHOR: Ebandao.cn                                    ##"
echo "## website: http://www.ebandao.cn    /                   ##"
echo "###########################################################"
echo "========================================================================="
echo ""
echo "LTMP status manage: /root/ltmp {start|stop|reload|restart|kill|status}"
...
InitInstall 2>&1 | tee /root/ltmp-install.log
CheckAndDownloadFiles 2>&1 | tee -a /root/ltmp-install.log
InstallDependsAndOpt 2>&1 | tee -a /root/ltmp-install.log
if [ "$isinstallmysql55" = "n" ]; then
	InstallMySQL51 2>&1 | tee -a /root/ltmp-install.log
else
	InstallMySQL55 2>&1 | tee -a /root/ltmp-install.log
fi
if [ "$isinstallphp53" = "n" ]; then
	InstallPHP52 2>&1 | tee -a /root/ltmp-install.log
else
	InstallPHP53 2>&1 | tee -a /root/ltmp-install.log
fi
InstallTengine 2>&1 | tee -a /root/ltmp-install.log
CreatPHPTools 2>&1 | tee -a /root/ltmp-install.log
AddAndStartup 2>&1 | tee -a /root/ltmp-install.log
CheckInstall 2>&1 | tee -a /root/ltmp-install.log
